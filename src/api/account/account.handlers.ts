...
export const getUserAccountDetails = factory.createHandlers(async (c) => {
  const account_id = (await getJWTPayload(c)).account_id;
...
export const updateAccountPassword = factory.createHandlers(
  validator("json", (value) => {
  ...
  async (c) => {
    ...
    const updatedAccount =
      await accountRepository.updateAccountPasswordByAccountId({
        accountId: account_id,
...
export const updateAccountEmail = factory.createHandlers(
  validator("json", (value) => {
  ...
  async (c) => {
    const dto = c.req.valid("json");
    const {account_id, email: oldEmail} = await getJWTPayload(c);
    ...
    if (
      (await compare(dto.password, passwordDetails.password ?? "")) === false
    ) {
      return c.json(
